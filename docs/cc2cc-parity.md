...
| 4 | Broadcast | a session fans a message out to all other sessions | `to:*` broadcast fans out to every currently-registered peer **except the sender**, one durable copy + own ack each; no late-joiner backfill. The broadcast copy is the sender's **verbatim signed envelope** (`to:"*"`, original id/HMAC) — per-recipient routing/ack rides on `wire.Deliver.delivery_key`, OUTSIDE the HMAC subset — so it verifies **end-to-end exactly like a direct message**. The parity test now **hard-asserts** real generic-bus delivery + end-to-end HMAC verify (no log-and-skip tolerance). | `TestParity_BroadcastFanOutSenderExcluded` |
| 5 | HMAC signing / trust | messages are HMAC-SHA256 signed; recipients reject forged ones | per-message HMAC over the canonical envelope carried end-to-end; recipient verifies before surfacing; a compromised broker cannot forge a peer. Holds for **both** direct and broadcast: the broker never mutates the signed envelope, so neither path is broker-trusted. | `TestParity_HMACSignedDeliversAndForgedRejected` |
